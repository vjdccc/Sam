...
async def gcast(event):
    ...
    if message:
        ...
        return await event.edit(
            "**⌔∮ يجب الرد على رسالة أو وسائط، أو كتابة نص مع الأمر.**"
        )

    # رسالة البداية مع عداد ديناميكي
    status_msg = await event.edit("**⌔∮ بدء الإذاعة إلى المجموعات...**")

    done = 0
    error = 0

    # جلب جميع المجموعات أولاً لحساب العدد الكلي (اختياري لكن جميل)
    groups = []
    async for dialog in event.client.iter_dialogs():
        if dialog.is_group and dialog.id not in GCAST_BLACKLIST:
            groups.append(dialog.id)

    total = len(groups)
    if total == 0:
        return await status_msg.edit("**⌔∮ لا توجد مجموعات متاحة للإذاعة!**")

    # الآن نبدأ الإرسال مع التحديث الحي
    for i, chat_id in enumerate(groups, start=1):
        try:
            await event.client.send_message(chat_id, msg)
            done += 1
        except (ChatWriteForbiddenError, UserBannedInChannelError, PeerIdInvalidError):
            error += 1
        ...
        # تحديث الرسالة كل 5 مجموعات (لتقليل عدد الطلبات وتجنب التقييد)
        if i % 5 == 0 or i == total:
            try:
                ...
                pass  # تجاهل إذا فشل التحديث (مثل تعديل رسالة محذوفة)

        await asyncio.sleep(0.3)  # تأخير صغير لتجنب Flood
